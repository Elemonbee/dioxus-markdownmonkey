...
**Version 0.6.3** · English | **[中文](./README.md)**
...
- 📝 **Markdown Editing** - **CodeMirror 6** is the source of truth (line numbers / Markdown coloring / fenced-code highlighting / kernel undo / fold headings and code); `/` snippets plus `](` link and image-path completion; hover preview and a light lint for missing local images; live preview (tables / strikethrough / task lists / footnotes / **`$` math** / **Mermaid**, loaded on demand); click a preview block to jump to its source line; clickable task checkboxes in the editor; raw HTML and dangerous URLs are filtered; **syntax-highlighted preview code blocks**
...
- 🗂️ **Multi-Tab** - Edit multiple files with independent kernel history per tab (undo survives tab switches); confirm before closing unsaved tabs
...
- 🤖 **AI Assistant** - OpenAI / Claude / DeepSeek / Kimi / Ollama / OpenRouter; API keys in the system keyring. **Toolbar / Ctrl+J** opens a live-context chat (prefers the current selection, otherwise a window around the cursor; character budget is configurable, default 4000). **Editor right-click** offers Copy / Cut / Paste plus selection-required presets (Continue, Improve, Outline, Translate to English/Chinese, Fix Grammar) that apply through a result modal. Streaming can be stopped; **chat history is per document**
...
- 🎨 **Themes & View** - Dark / Light / Follow System; persisted window size; word wrap and line numbers follow Settings; optional editor–preview sync scroll (Settings and preview pane)
...
assets/
├── branding/               # app icon PNGs (window / README)
...
Cross-platform packaging and tagging are documented in **[docs/RELEASE.md](docs/RELEASE.md)** (GitHub Actions: Windows zip + Setup.exe / Linux tar.gz + .deb / macOS `.app`). Complete **[docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md)** before tagging `v0.6.3`.
